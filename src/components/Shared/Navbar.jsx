...
const Navbar = () => {
  ...
  return (
    <div className='w-screen'>
        <div className={`fixed w-[85%] md:w-[90%] lg:w-3/4 xl:w-3/5 border-black border-2 mt-5 z-50 right-0 left-0 mx-auto rounded-3xl backdrop-blur-sm duration-300 shadow-md overflow-hidden ${navbar ? 'h-[400px]' : 'h-[80px]'}`}>
        ...
        {/* searchbar */}
        {search && (
            <div className='fixed flex items-center w-[95%] md:w-[90%] h-[50px] rounded-2xl lg:w-3/4 xl:w-3/5 shadow-md mx-auto right-0 left-0 top-[12%] translate-y-4 transform float-up'>
                <input type="text" placeholder='Search an item...' className='w-[90%] h-full rounded-l-2xl pl-4 border-r-0 border-2' />
                <button className='border-2 h-full w-[10%] rounded-r-2xl hover:bg-amber hover:text-white hover:border-black'>Search</button>
            </div>
        )}
    </div>
  )
