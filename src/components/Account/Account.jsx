...
const Account = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put('https://adminaliyaumbrella.worldpos.biz/api/Customer', customerData, {
        headers: {
    ...
    <div className='w-full min-h-[90vh] pt-28 bg-white'>
      <div className='w-[80%] mx-auto pt-5'>
        ...
        <button 
          ...
          className='px-5 py-2 border border-red text-red hover:border-amber hover:bg-third duration-100 mb-10 md:mb-0 ml-3'>
          Log Out
        </button>
        ...
        <Dialog.Root open={openUserModal} onOpenChange={setOpenUserModal}>
          ...
          <Dialog.Content className="fixed max-h-[95vh] overflow-y-auto z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-5 rounded-md w-full max-w-[90%] md:max-w-[70%] lg:max-w-[50%]">
            <Dialog.Title className="text-xl font-bold">Edit User Information</Dialog.Title>
            ...
            <div className="flex justify-end mt-4">
              <button onClick={handleSubmit} className="px-4 py-2 bg-amber text-white rounded-md hover:bg-yellow-600 transition">
                Save Changes
              </button>
            </div>
          </Dialog.Content>
